...
class NimAI:
    ...
    def choose_action(self, state, epsilon=True):
        ...
        for action in actions:
            if (tuple(state), action) in self.q:
                qvals[action] = self.q[tuple(state), action]
            else:
                qvals[action] = 0
        max_action = max(qvals, key=lambda k: qvals[k])
        if epsilon is False:
            return max_action
        else:
            if (
                random.choices(
                    [max_action, "random"], weights=[1 - self.epsilon, self.epsilon], k=1
                )[0]
                == "random"
            ):
                return random.choice(list(qvals.keys() - {max_action}))
            else:
                return max_action
